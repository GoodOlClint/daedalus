...
# so the daedalus user can read it; --node-name labyrinth for clarity.
...
echo
echo "    scp daedalus@<labyrinth-ip>:/etc/rancher/k3s/k3s.yaml ~/.kube/daedalus.yaml"
echo "    sed -i '' 's/127.0.0.1/<labyrinth-ip>/' ~/.kube/daedalus.yaml   # GNU: drop the ''"
echo "    KUBECONFIG=~/.kube/daedalus.yaml kubectl get nodes"
