...
func (s *Store) TransitionTask(ctx context.Context, id uuid.UUID, to storage.State) error {
	...
	case storage.StateRunning:
		q = `UPDATE minos.tasks SET state = $1, started_at = $3 WHERE id = $2`
		args = append(args, now)
	case storage.StateCompleted, storage.StateFailed:
		q = `UPDATE minos.tasks SET state = $1, finished_at = $3 WHERE id = $2`
		args = append(args, now)
	default:
		q = `UPDATE minos.tasks SET state = $1 WHERE id = $2`
	}
